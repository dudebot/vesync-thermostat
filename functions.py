...
def set_device_state(turn_on):
    ...
    if not found:
        print(f"Device {config['device']} not found. Please delete config.txt and re-run")
...
async def getweather():
    ...
    weather = await client.find("Dallas TX")

    # returns the current day's forecast temperature (int)
    result['current_temp'] = weather.current.temperature
